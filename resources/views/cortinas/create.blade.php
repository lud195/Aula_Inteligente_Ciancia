...
<form action="{{ route('cortinas.store') }}" method="POST">
    ...
    <option value="baja" {{ old('nivel_cortina') == 'baja' ? 'selected' : '' }}>Baja</option>
    <option value="media" {{ old('nivel_cortina') == 'media' ? 'selected' : '' }}>Media</option>
    <option value="alta" {{ old('nivel_cortina') == 'alta' ? 'selected' : '' }}>Alta</option>
</select>
